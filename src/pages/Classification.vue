<template>
  <div class="classificationContainer">
    ...
    <el-card class="classificationBox">
      ...
      <!-- 表格 -->
      <el-table
        :data="classificationData.slice((currentPage - 1) * pageSize,currentPage * pageSize)"
        border
        stripe
        style="width: 100%"
      >
...
export default {
  ...
  methods: {
    ...
    addClassification() {
      ...
      this.addClassData.createTime = this.$moment(dateTime).format('YYYY-MM-DD HH:DD:SS')
    ...
    searchClassName() {
      ...
      getRequest('/classify/search', {className: this.keyword}).then(resp => {
        console.log(resp);
        if(resp.code !== 0) {
    ...
    submitEditClassify() {
      const dateTime = new Date()
      this.editClassData.updateTime = this.$moment(dateTime).format('YYYY-MM-DD HH:DD:SS')
